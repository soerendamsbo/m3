...
- Vi finder erstatningstime vha. poll i pausen (dagen før eller dagen efter)
...
- Antagese: Effekten af X på Y er konstant (effekten af $X_1$ er $\beta_1$) uanset hvilke værdier de øvrige uafhængige variable i modellen har
...
- Man kan alle være "for tydelig" med brug af "i." og "c."

-

- Måleniveauer er også afgørende for fortolkningen &rarr; det ser vi i øvelsesopgaverne om lidt
...
- **Fortegn**: Retning som hovedeffekten (X &rarr; Y) bevæger sig, når den modererende variabel (Z) ændres med +1
    - &rarr; *større eller mindre*?

- **Størrelse**: *Hvor meget* hovedeffekten (X &rarr; Y) ændrer sig, når den moderende variabel (Z) vokser med +1

--

- **Beta-koefficienten for X-hovedleddet**: Hvordan påvirker X &rarr; Y, når den modererende variabel er $Z = 0$ &rarr; kan ses som "udgangspunktet" for effekten
    - Det er en **marginal effekt** &rarr; effekten for en bestemt gruppe 
    - Bemærk at koefficienterne for de konstituerende led ikke altid har en meningsfuld tolkning (Stata og *margins* hjælper os om lidt)
...
- Hvis Z er **kategorisk** med mere end to kategorier: 
    ...
    - Der er et interaktionsled (X * Z) for hver af disse "sammenligninger" 
...
- Den mest tricky - by far! - er, når **både X og Z er kategoriske**. Det følger fuldstændigt samme princip, så det er altså ikke umuligt. Men tungen skal holdes lige i munden, fordi der er mere end én "hovedeffekt" (X &rarr; Y), som så varierer forskelligt for forskellige niveauer af Z. Den springer vi over for nu.
...
- (Vær varsom med at *ekstrapolere*, dvs. beregne marginal effekt for subjekter, som ikke findes i data eller måske overhovedet, fx en -200-årig)

--

Det hjælper ofte (faktisk altid!) at visualisere de marginale effekter. Det gør ved at køre *marginsplot* umiddelbart efter *margins*: 
...
- Vi laver de første 2-3 opgaver sammen

- Derefter laver I resten i grupper (4-7)
    - Break-out sessions (ca. 20 min.)
    - I sidder selv og laver opgaverne i Stata (brug evt. screen-share)
    - Muligvis hopper jeg lidt rundt i rummene
    
- Vi gennemgår og fortolker sammen: 
