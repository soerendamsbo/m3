...
- Måleniveauer er vigtige for udførelse og fortolkning (intervalskaleret, kategorisk, binær)
...
- Interaktionsleddet er *forskellen i effekt* eller *noget ekstra, der lægges til effekten* &rarr; signifikant?

- Fortolkning kan være udfordrende - især med kategoriske variable
...
- OLS estimerer den rette linje, som minimerer de kvadrerede residualer 
    ...
    - ret linje &rarr; linearitet &rarr; konstant hældning 
    
    - hældning = *beta-koefficient*
    
--

(Blandt andet) derfor er OLS fedt:
...
- Det komplicerer vores tolkning markant: 
    ...
    - Hvorvidt/hvordan en ændring i X påvirker sandsynligheden for, at Y=1 **afhænger af, hvor på x-aksen vi står**

--

Kan vi matematisk udtrykke disse heterogene effekter med ét tal, som i OLS?

- Spoiler alert ... Det kan vi!

- Men dette ene tal ("logit") er ikke intuitivt

- Heldigvis er der løsninger, når vi kommer til fortolkning (fx AME/MEM)
...
Hvordan og hvor godt beskriver de data (dvs. sammenhængen mellem X og Y)?
...
- Den funktionelle form (linearitet) er problematisk &rarr; gennemsnitseffekt er upassende
...
- Vi indsætter vores lineære sandsynlighedsmodel på X's plads, så vi får:

$Y = P(Y = 1 | X) = F(\alpha+\beta X+\epsilon) = \frac{e^{\alpha+\beta X+\epsilon}}{1+e^{\alpha+\beta X+\epsilon}}$

- Modellen giver os sandsynligheden for at Y = 1 som en *logistisk funktion af X*
...
# Opsummering af OLS versus logit

- Tænk på logistisk regression som **en udvidelse af lineær regression** (dog en udvidelse til en ikke-lineær model)
...
--
    ...
    - Hvis man laver kausalstudier i praksis, er eksogenitetsantagelsen (fravær af spuriøsitet m.m.) langt vigtigere end valget mellem OLS vs. logistisk regression
...
## Sandsynlighed, odds, logit-koefficient og odds ratio
...
- Stata giver os som standard logit-koefficienten
...
- De fleste er nok enige i, at den mest intuitive fortolkning er:
...
- Marginal Effect at the Mean (MEM)
    - Den marginale effekt ved gennemsnittet af X
...
- Kan også være mere eller mindre smart - *mere eller mindre repræsentativ for den samlede effekt*
    - afhængiger bl.a. af S-kurvens hældning og den konkrete X-variabel
...
I nogle tilfælde er AME og MEM nærmest ens som i vores tilfælde
...
- AME og MEM divergerer, når sammenhængen er meget S-formet (MEM > AME)

- Jo mere lineær, vores S-kurve er, jo mere ens vil de være
...
- Hvorfor forsøge så hårdt at opsummere hele den ikke-lineære model i ét tal, når vi bare kan **plotte den forudsagte sandsynlighed for Y = 1 for alle værdier af X**? 

--

    - giver et hurtigt **overblik**: forudsagt Y (sandsynlighed) på Y-aksen, forklarende X på X-aksen
    
--

    - vi kan vurdere hele sammenhængen
...
Fra (fx) 20 til 70 år stiger den forudsagte sandsynlighed for at stemme personligt med ca. 20 procentpoint, hvilket svarer til vores AME, som vi gangede op til 50 år. Effekten er lille på årsbasis, men løber op over tid

Desuden næsten en ret linje: 

S-formen er ikke tydelig

Mao. er linearitet ikke en problematisk funktionel form for sandsynligheden i dette tilfælde
...
- Linearitet ("linear in the logits") &rarr; der skal være en lineær sammenhæng mellem Y udtrykt som log-odds og den uafhængige variabel X &rarr; tjek jf. Sønderskov (2014), pp. 274ff 
