...
- De første har haft deres første vejledning &rarr; datoer: 16/4, 19/4, 23/4

- Husk at tilmelde jer i Google Sheets &rarr; [http://kortlink.dk/2b3zy](https://docs.google.com/spreadsheets/d/1E81aUlid_0q6mMuxVZyLYhfcqZhQybu1pwlxlALtQn4/edit)
    - de fleste har gjort det &rarr; skriv til mig, hvis der ikke er flere tider
...
- **Paneldata** er en datatype med gentagne observationer af mange enheder (fx survey to gange med samme respondenter)
...
- At kunne analysere og fortolke resultaterne af et eksperiment (nemt)
...
Grundsten i moderne eksperimentel politologi og socialvidenskab (eksperiment- og kausal inferens-interesserede kan med fordel læse lidt videre)

---

# Hvad er et eksperiment*?

*aka _randomized controlled trials_ (RCTs)
...
- Kort fortalt er eksperimentet en metode, hvor man **tilfældigt fordeler nogle subjekter** i (mindst) to grupper - en treatmentgruppe og en kontrolgruppe - og tildeler subjekterne i treatmentgruppen et - I gættede det - **treatment** (stimulus)
    ...
    - Pga. randomiseringen er grupperne statistisk set. Treatment er den eneste forskel. **Derfor kan effekten af treatment estimeres som forskellen i de to gruppers outcome**
...
- Potential outcomes er en måde at formalisere den kausale logik (hvad det reelt betyder, at X har en effekt på Y) 
...
- Hvert subjekt $i$ har to potentielle udfald på outcome (den afhængige variabel) $Y_i$:
...
- Mere præcist betyder det, at der er lige stor sandsynlighed for at blive "udtrukket" til treatment for kvinder, mænd, unge, gamle, rige, fattige, høje, lave, hvide, sorte, universitetsuddannede, faglærte etc. etc. 
    ...
    - mao. er grupperne i forventningen ens &rarr; statistisk set ens 
...
- I økonometri-lingo er den **uafhængige variabel (*X*) uafhængig af fejlledet**, $\epsilon_i$. Mao. varierer $\epsilon_i$ blot tilfældigt ift. $X_i$ i modellen $Y_i = \alpha + \beta_1 X_i + \epsilon_i$
...
- **Simpel randomisering** betyder blot, at der er 20 % sandsynlighed for at hvert subjekt placeres i kontrolgruppen, men reelt kan det (tilfældigvis) ende med at være 12, 17 eller 25   
...
- At vi kontrollerer for samtlige potentielt relevante variable:  
    ...
    - at vi *faktisk* har observeret (målt) dem (vi har data)
...
__Antagelse: Treatmenttildelingen påvirker kun Y gennem selve treatmentet__
...
**Antagelse: Ingen subjekter må påvirkes af _andre subjekters_ treatmentstatus - ingen spill-over mellem subjekter**
...
- Endnu en grund til blinding (og evt. at adskille subjekter i rum/tid)
...
- Den unbiased **effekt** af et treatment estimeres simpelthen som forskellen i gennemsnittet af outcome mellem treatment- og kontrolgruppen (*difference-in-means*)
...
2. Men vi kan lave en **balancetest**, der viser, om grupperne empirisk set er ens (balancerede) på en række baggrundsvariable, fx køn (*gender*), alder (*age*), uddannelse (*profile_education*) og pre-treatment holdning (*ku1*)

3. Hvis vi lavede en **regression af treatment på de fire baggrundsvariable** (obs! treatment er Y), og vi fandt en **statistisk signifikant sammenhæng mellem alder og treatment**, 

    a. *hvad ville dette så fortælle os* om sammensætningen af vores treatment- vs. kontrolgrupper?
    
    b. ville vi kunne se, om randomiseringen var succesfuld? 
...
- Vi skal have fat i **difference-in-means** &rarr; den estimerede effekt er simpelthen forskellen i gennemsnit mellem hver treatmentgruppe og kontrolgruppen

- **Hvordan finder vi fx effekten af at blive treated med vignette 3?**
...
**En sidste pointe om kontrolvariable i eksperimenter.** Man kan godt medtage en eller flere **kontrolvariable** i sin regression, selvom det er et eksperiment uden OVB. Dette kan øge **præcisionen** af estimaterne (mindre standardfejl) uden at tilføre bias, især **hvis grupperne er ubalancerede** (uens trods den tilfældige fordeling). Man skal dog for det første **undgå at kontrollere for faktorer, der selv kan være påvirket af treatment** (&rarr; *post-treatment bias*). For det andet skal man ikke bruge det til **p-hacking**. Ideelt set planlægger man på forhånd, hvilke kontrolvariable man evt. vil inddrage, og uanset hvad **fremlægger man også den "rene" analyse** uden kontrolvariable (*se eksempel i do-fil*)
...
- Eksperimenter er med god grund guldstandarden for kausal inferens
...
- **Analysen** er nem &rarr; typisk laver vi blot **OLS** med outcome som Y og treatmentgruppe (kategorisk) som X
